...
class _AccessoriesScreenState extends State<AccessoriesScreen> {
  ...
  void _listenInventory() {
    _invSub = _db
        .collection('accessories_$_ownerID')
        .orderBy('timestamp', descending: true)
        .snapshots()
        .listen((snap) {
      final list = snap.docs.map((d) => {'id': d.id, ...d.data()}).toList();
      if (mounted) setState(() { _inventory = list; _loaded = true; _filter(); });
    });
  }
  ...
  void _openSearchScanner() {
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => _AccBarcodeScannerPage(
  ...
  Future<void> _handleScannedCode(String code) async {
    ...
    // Always create separate stock entry — never increment qty
    // Pre-fill kod + copy nama/kos/jual from existing item if any
            ...
            child: SingleChildScrollView(child: Column(
              ...
              children: [
                ...
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Row(children: [
                    const FaIcon(FontAwesomeIcons.boxOpen, size: 14, color: AppColors.primary),
                    const SizedBox(width: 8),
                    Text(_lang.get('st_tambah_stok'), style: const TextStyle(color: AppColors.primary, fontSize: 13, fontWeight: FontWeight.w900)),
                  ]),
                  GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: pickedImage != null
                        ...
                        : Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                            const FaIcon(FontAwesomeIcons.camera, size: 24, color: AppColors.textDim),
                            const SizedBox(height: 6),
                            Text(_lang.get('st_tambah_gambar'), style: const TextStyle(color: AppColors.textDim, fontSize: 9, fontWeight: FontWeight.w700)),
                          ]),
                  ),
                ),
                ...
                Padding(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(_lang.get('st_kategori'), style: const TextStyle(color: AppColors.textSub, fontSize: 10, fontWeight: FontWeight.w900)),
                    ...
                    Container(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          ...
                          items: [
                            DropdownMenuItem(value: 'SPAREPART', child: Text(_lang.get('st_sparepart'))),
                            DropdownMenuItem(value: 'ACCESSORIES', child: Text(_lang.get('st_accessories'))),
                          ],
                ...
                SizedBox(
                  ...
                  child: uploading
                    ...
                    onTap: () async {
                      ...
                      await _db.collection('accessories_$_ownerID').add({
              ...
              children: [
                ...
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Expanded(child: Row(children: [
                    ...
                    const SizedBox(width: 8),
                    Flexible(child: Text('${_lang.get('st_edit')}: $kod', style: const TextStyle(color: AppColors.yellow, fontSize: 13, fontWeight: FontWeight.w900), overflow: TextOverflow.ellipsis)),
                  ])),
                ...
                // Update & Print label (1 row, compact)
                Row(children: [
                  Expanded(
                    child: _buildGlassButton(
                      ...
                      color: AppColors.primary,
                      onTap: () async {
                        await _db.collection('accessories_$_ownerID').doc(docId).update({
                ...
                Row(children: [
                  ...
                  Text(_lang.get('st_reverse_stock'), style: const TextStyle(color: AppColors.red, fontSize: 11, fontWeight: FontWeight.w900)),
                ]),
                const SizedBox(height: 8),
                Row(children: [
                  Expanded(child: _formField('No. Siri Job', reverseSiriCtrl, 'Cth: RMS-00001')),
                  const SizedBox(width: 8),
                  _actionBtn(FontAwesomeIcons.arrowsRotate, _lang.get('reverse'), AppColors.red, () async {
                    ...
                    onTap: () async {
                      final confirm = await showDialog<bool>(
                        ...
                        builder: (dCtx) => AlertDialog(
                          ...
                          title: Text(_lang.get('st_padam_item'), style: const TextStyle(color: AppColors.red, fontSize: 14, fontWeight: FontWeight.w900)),
                          content: Text('${_lang.get('st_pasti_padam')} "$kod"?', style: const TextStyle(color: AppColors.textSub, fontSize: 12)),
                          actions: [
                            TextButton(onPressed: () => Navigator.pop(dCtx, false), child: Text(_lang.get('batal'), style: const TextStyle(color: AppColors.textMuted))),
                            ElevatedButton(
                              onPressed: () => Navigator.pop(dCtx, true),
                              style: ElevatedButton.styleFrom(backgroundColor: AppColors.red),
                              child: Text(_lang.get('padam'), style: const TextStyle(color: Colors.white)),
                            ),
                          ],
                        ),
                      );
                      if (confirm == true) {
                        await _db.collection('accessories_$_ownerID').doc(docId).delete();
                        if (ctx.mounted) Navigator.pop(ctx);
  ...
  Future<List<Map<String, dynamic>>> _loadUsedHistory() async {
    final snap = await _db.collection('acc_usage_$_ownerID')
            ...
            child: Column(children: [
              Padding(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Row(children: [
                    const FaIcon(FontAwesomeIcons.clockRotateLeft, size: 14, color: AppColors.orange),
                    const SizedBox(width: 8),
                    Text(_lang.get('st_history_used'), style: const TextStyle(color: AppColors.orange, fontSize: 13, fontWeight: FontWeight.w900)),
                  ]),
                  GestureDetector(onTap: () => Navigator.pop(ctx), child: const FaIcon(FontAwesomeIcons.xmark, size: 16, color: AppColors.red)),
                ]),
              ),
              ...
              Expanded(
                child: usedList == null
                    ? const Center(child: CircularProgressIndicator(color: AppColors.primary))
                    : usedList!.isEmpty
                        ? Center(child: Text(_lang.get('st_tiada_rekod_guna'), style: const TextStyle(color: AppColors.textDim, fontSize: 12)))
                        : ListView.builder(
                                ...
                                child: Row(children: [
                                  ...
                                  GestureDetector(
                                    onTap: () async {
                                      ...
                                      final usageId = d['_id'] ?? '';
                                      await _db.collection('accessories_$_ownerID').doc(stockDocId).update({
                                        'status': 'AVAILABLE',
                                        'tkh_guna': '',
                                      });
                                      await _db.collection('acc_usage_$_ownerID').doc(usageId).update({
                                        'status': 'REVERSED',
                                    ...
                                    child: Container(
                                      ...
                                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                                        const FaIcon(FontAwesomeIcons.arrowsRotate, size: 9, color: AppColors.blue),
                                        const SizedBox(width: 4),
                                        Text(_lang.get('reverse'), style: const TextStyle(color: AppColors.blue, fontSize: 8, fontWeight: FontWeight.w900)),
                                      ]),
                                    ),
                                  ),
                                ]),
            ...
            child: Column(children: [
              Padding(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Row(children: [
                    ...
                    const SizedBox(width: 8),
                    Text(_lang.get('st_history_return'), style: const TextStyle(color: AppColors.red, fontSize: 13, fontWeight: FontWeight.w900)),
                  ]),
                  GestureDetector(onTap: () => Navigator.pop(ctx), child: const FaIcon(FontAwesomeIcons.xmark, size: 16, color: AppColors.red)),
                ]),
              ),
              ...
              Expanded(
                child: returnList == null
                    ? const Center(child: CircularProgressIndicator(color: AppColors.primary))
                    : returnList!.isEmpty
                        ? Center(child: Text(_lang.get('st_tiada_rekod_return'), style: const TextStyle(color: AppColors.textDim, fontSize: 12)))
                        : ListView.builder(
                            padding: const EdgeInsets.all(12),
                                ...
                                child: Row(children: [
                                  ...
                                  GestureDetector(
                                    onTap: () async {
                                      ...
                                      final stockSnap = await _db.collection('accessories_$_ownerID').doc(stockDocId).get();
                                      if (stockSnap.exists) {
                                      ...
                                      await _db.collection('accessories_$_ownerID').doc(stockDocId).collection('returns').doc(returnDocId).delete();
  ...
  Widget _buildHeader() {
    return Container(
      ...
      child: Row(children: [
        const FaIcon(FontAwesomeIcons.boxesStacked, size: 14, color: AppColors.primary),
        const SizedBox(width: 8),
        Text(_lang.get('st_stock_cn'), style: const TextStyle(color: AppColors.primary, fontSize: 14, fontWeight: FontWeight.w900, letterSpacing: 1)),
        const Spacer(),
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              const FaIcon(FontAwesomeIcons.clockRotateLeft, size: 10, color: AppColors.orange),
              const SizedBox(width: 5),
              Text(_lang.get('st_h_used'), style: const TextStyle(color: AppColors.orange, fontSize: 9, fontWeight: FontWeight.w900)),
            ]),
          ),
        ),
        const SizedBox(width: 6),
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              const FaIcon(FontAwesomeIcons.rotateLeft, size: 10, color: AppColors.red),
              const SizedBox(width: 5),
              Text(_lang.get('st_h_return'), style: const TextStyle(color: AppColors.red, fontSize: 9, fontWeight: FontWeight.w900)),
            ]),
          ),
        ),
        const SizedBox(width: 6),
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(6),
              boxShadow: [BoxShadow(color: AppColors.green.withValues(alpha: 0.4), blurRadius: 8, offset: const Offset(0, 3))],
            ),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
  ...
  Widget _buildSearchBar() {
    return Container(
      ...
      child: TextField(
        ...
        style: const TextStyle(color: AppColors.textPrimary, fontSize: 12),
        decoration: InputDecoration(
          hintText: _lang.get('st_cari_hint'),
          hintStyle: const TextStyle(color: AppColors.textDim, fontSize: 12),
    ...
    if (!_loaded) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 16),
          Text(_lang.get('st_memuatkan'), style: TextStyle(color: AppColors.textMuted, fontSize: 12, fontWeight: FontWeight.w700)),
        ]),
      );
    }
    if (_filtered.isEmpty) {
      return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        const FaIcon(FontAwesomeIcons.boxOpen, size: 40, color: AppColors.textDim),
        const SizedBox(height: 12),
        Text(_lang.get('st_tiada_item'), style: const TextStyle(color: AppColors.textMuted, fontWeight: FontWeight.w700)),
      ]));
    }
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Container(
            width: 44, height: 44,
            decoration: BoxDecoration(
              color: qty <= 2 ? AppColors.red.withValues(alpha: 0.12) : AppColors.primary.withValues(alpha: 0.1),
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: qty <= 2 ? AppColors.red.withValues(alpha: 0.3) : AppColors.primary.withValues(alpha: 0.2)),
            ),
            child: Center(child: Text('$qty', style: TextStyle(
              color: qty <= 2 ? AppColors.red : AppColors.primary, fontSize: 16, fontWeight: FontWeight.w900))),
          ),
          const SizedBox(width: 12),
          // Content
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: Text(kod, style: const TextStyle(color: AppColors.primary, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 0.5))),
            ...
            if (tarikhMasuk.isNotEmpty || tkhJual.isNotEmpty || siriJual.isNotEmpty)
              Padding(
                ...
                child: Row(children: [
                  if (tarikhMasuk.isNotEmpty) Text('${_lang.get('st_masuk')}: $tarikhMasuk', style: const TextStyle(color: AppColors.textDim, fontSize: 8)),
                  if (tkhJual.isNotEmpty) ...[
                    const SizedBox(width: 8),
                    Text('${_lang.get('st_jual')}: $tkhJual', style: const TextStyle(color: AppColors.textDim, fontSize: 8)),
                  ],
  ...
  Widget _buildFooter() {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 6),
        Expanded(child: _footerChip('Jual: RM ${totalJual.toStringAsFixed(0)}', AppColors.green)),
      ]),
    );
  }
...
class _AccBarcodeScannerPage extends StatefulWidget {
  final void Function(String code) onScanned;
  const _AccBarcodeScannerPage({required this.onScanned});

  @override
  State<_AccBarcodeScannerPage> createState() => _AccBarcodeScannerPageState();
}

class _AccBarcodeScannerPageState extends State<_AccBarcodeScannerPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(AppLanguage().get('scan_qr_barcode'), style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w900, letterSpacing: 1)),
        centerTitle: true,
        leading: IconButton(
          ...
          Positioned(
            ...
            child: Center(
              child: Container(
                ...
                child: const Text(
                  'Halakan kamera ke QR Code atau Barcode',
