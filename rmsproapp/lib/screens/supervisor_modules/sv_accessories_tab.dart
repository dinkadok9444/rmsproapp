...
class SvAccessoriesTab extends StatefulWidget {
  ...
  const SvAccessoriesTab({super.key, required this.ownerID, required this.shopID});
  @override
  State<SvAccessoriesTab> createState() => _SvAccessoriesTabState();
}
...
class _SvAccessoriesTabState extends State<SvAccessoriesTab> {
  ...
  static const _accentColor = Color(0xFF8B5CF6); // purple
  ...
  void _showAddModal({String? prefillKod, String? prefillNama, double? prefillKos, double? prefillJual}) {
            ...
            child: SingleChildScrollView(child: Column(
              ...
              children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  const Row(children: [
                    FaIcon(FontAwesomeIcons.headphones, size: 14, color: _accentColor),
                    SizedBox(width: 8),
                    Text('TAMBAH ACCESSORIES', style: TextStyle(color: _accentColor, fontSize: 13, fontWeight: FontWeight.w900)),
                  ]),
                  GestureDetector(
                ...
                // Kod with copy, auto-generate & scan
                Row(children: [
                  Expanded(child: _formField('Kod Item', _kodCtrl, 'Cth: ACC-001')),
                ...
                _formField('Nama Item', _namaCtrl, 'Cth: Casing iPhone 15'),
              ...
              children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Expanded(child: Row(children: [
                    const FaIcon(FontAwesomeIcons.penToSquare, size: 14, color: _accentColor),
                    const SizedBox(width: 8),
                    Flexible(child: Text('EDIT: $kod', style: const TextStyle(color: _accentColor, fontSize: 13, fontWeight: FontWeight.w900), overflow: TextOverflow.ellipsis)),
                  ])),
  ...
  Future<void> _printLabel(Map<String, dynamic> item) async {
    ...
    var r = escInit;
    r += escCenter;
    r += '$escBoldOn${escDblSize}LABEL ACC\n$escNormal$escBoldOff';
    r += '${'=' * 32}\n';
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (_, i) {
                              ...
                              return Container(
                                ...
                                child: Row(children: [
                                  const FaIcon(FontAwesomeIcons.headphones, size: 14, color: AppColors.orange),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: _accentColor)),
          ),
        ),
      ]),
    );
  ...
  Widget _buildHeader() {
    return Container(
      ...
      child: Row(children: [
        const FaIcon(FontAwesomeIcons.headphones, size: 14, color: _accentColor),
        const SizedBox(width: 8),
        const Text('ACCESSORIES', style: TextStyle(color: _accentColor, fontSize: 14, fontWeight: FontWeight.w900, letterSpacing: 1)),
        const Spacer(),
        GestureDetector(
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: _accentColor,
              borderRadius: BorderRadius.circular(6),
              boxShadow: [BoxShadow(color: _accentColor.withValues(alpha: 0.4), blurRadius: 8, offset: const Offset(0, 3))],
            ),
            child: const Row(mainAxisSize: MainAxisSize.min, children: [
              FaIcon(FontAwesomeIcons.plus, size: 10, color: Colors.white),
              SizedBox(width: 5),
              Text('TAMBAH', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w900)),
            ]),
          ),
        ),
      ]),
  ...
  Widget _buildSearchBar() {
    return Container(
      ...
      child: TextField(
        ...
        style: const TextStyle(color: AppColors.textPrimary, fontSize: 12),
        decoration: InputDecoration(
          hintText: 'Cari kod / nama accessories...',
          hintStyle: const TextStyle(color: AppColors.textDim, fontSize: 12),
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: const Padding(
                  ...
                  child: FaIcon(FontAwesomeIcons.barcode, size: 14, color: _accentColor),
                ),
              ),
            ],
          ),
    ...
    if (_inventory.isEmpty) {
      return const Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          SizedBox(height: 16),
          Text('Memuatkan accessories...', style: TextStyle(color: AppColors.textMuted, fontSize: 12, fontWeight: FontWeight.w700)),
        ]),
      );
    }
    if (_filtered.isEmpty) {
      return const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        FaIcon(FontAwesomeIcons.headphones, size: 40, color: AppColors.textDim),
        SizedBox(height: 12),
      ...
      child: Container(
        ...
        child: Row(children: [
          Container(
            width: 44, height: 44,
            decoration: BoxDecoration(
              color: qty <= 2 ? AppColors.red.withValues(alpha: 0.12) : _accentColor.withValues(alpha: 0.1),
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: qty <= 2 ? AppColors.red.withValues(alpha: 0.3) : _accentColor.withValues(alpha: 0.2)),
            ),
            child: Center(child: Text('$qty', style: TextStyle(
              color: qty <= 2 ? AppColors.red : _accentColor, fontSize: 16, fontWeight: FontWeight.w900))),
          ),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: Text(kod, style: const TextStyle(color: _accentColor, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 0.5))),
              Container(
            ...
            Row(children: [
              ...
              if (supplier.isNotEmpty) ...[
                const SizedBox(width: 8),
                Flexible(child: Text(supplier, style: const TextStyle(color: _accentColor, fontSize: 8, fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis)),
              ],
            ]),
